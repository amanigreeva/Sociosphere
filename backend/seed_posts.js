...
async function seedPosts() {
    try {
        ...
        for (const [category, posts] of Object.entries(samplePosts)) {
            ...
            for (const post of posts) {
                try {
                    ...
                    console.log(`  ✓ Created: ${post.text.substring(0, 50)}...`);
                } catch (err) {
        ...
        console.log(`📊 Total posts created: ${Object.values(samplePosts).flat().length}`);

    } catch (error) {
        console.error('❌ Error seeding posts:', error.response?.data || error.message);
    }
}
