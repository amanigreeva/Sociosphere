...
const seedReels = async () => {
    try {
        ...
        // Delete existing posts to start fresh (optional, but good for demo)
        ...
        for (const reel of sampleReels) {
            const newReel = new Post({
                ...
                text: reel.text,
                media: [reel.video],
                category: reel.category, // Even if UI defaults to OTHER, backend supports others
                likes: [],
                comments: []
            });
            await newReel.save();
        ...
        console.log('Successfully seeded 5 Instagram-style reels.');
        process.exit(0);
    } catch (err) {
        console.error(err);
        process.exit(1);
